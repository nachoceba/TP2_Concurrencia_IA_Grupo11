...
VALUES ('Empanadas', 'Variedad de empanadas artesanales')
RETURNING id;

-- HU-CAT-03: Editar categoría
UPDATE categoria
SET    nombre = 'Pizzas y Empanadas', descripcion = 'Catálogo ampliado de pizzería'
...
-- HU-PROD-01: Listar productos vigentes
...
SELECT 'Fugazzeta', 'Pizza rellena de queso y cebolla', 1800.00, 10, NULL, TRUE, c.id
FROM   categoria c
...
VALUES ('Juan', 'Pérez', 'juan.perez@example.com', '2611234567', 'hash_pass_123')
RETURNING id;
...
-- HU-PED-03: Actualizar estado y forma de pago de un pedido
UPDATE pedido
SET    estado = 'CONFIRMADO', forma_pago = 'TARJETA'
WHERE  id = 1 AND eliminado = FALSE;

-- HU-PED-04: Eliminar pedido (baja lógica encadenada)
BEGIN;
...
-- A) Top 5 productos más vendidos (por cantidad de unidades)
...
-- B) Facturación acumulada por categoría y por mes
SELECT c.nombre AS categoria,
       date_trunc('month', ped.fecha) AS mes,
       SUM(dp.subtotal) AS facturado
FROM   detalle_pedido dp
JOIN   pedido   ped ON ped.id = dp.producto_id AND ped.eliminado = FALSE
JOIN   producto pr  ON pr.id  = dp.producto_id
...
-- E) Productos sin ventas registradas (LEFT JOIN + IS NULL)
